...
from data.ClkData import ClkData, NewClkData, OldClkData
from utility.util import LogLevel, log, sorted_key
...
class ClkObj(ModuleObj):
    ...
    def read(self, node):
        ...
        for node in nodes:
            if node.nodeType == xml.dom.Node.ELEMENT_NODE:
                ...
                key = (
                    re.findall(r"\D+", node.nodeName)[0].upper()
                    + self._suffix
                    + "%s" % (re.findall(r"\d+", node.nodeName)[0])
                )
    ...
    def fill_hFile(self):
        ...
        for key in sorted_key(list(ModuleObj.get_data(self).keys())):
            ...
            if idx >= 0:
                gen_str += (
                    """#define %s_DRIVING_CURR\t\tCLK_BUF_DRIVING_CURR_%d\n"""
                    % (key, idx)
                )
            else:
                gen_str += (
                    """#define %s_DRIVING_CURR\t\tCLK_BUF_DRIVING_CURR_AUTO_K\n"""
                    % (key)
                )
...
class ClkObj_Everest(ClkObj):
    ...
    def fill_hFile(self):
        ...
        for key in sorted_key(list(ModuleObj.get_data(self).keys())):
            ...
            if key.find(self.__pmic) != -1:
                gen_str += (
                    """#define %s_DRIVING_CURR\t\tCLK_BUF_DRIVING_CURR_%sMA\n"""
        ...
        for node in nodes:
            if node.nodeType == xml.dom.Node.ELEMENT_NODE:
                ...
                if len(cur_driving_node):
                    data.cur_driving_control = (
                        cur_driving_node[0].childNodes[0].nodeValue
                    )
    ...
    def fill_hFile(self):
        ...
        for key in sorted_key(list(ModuleObj.get_data(self).keys())):
            ...
            if len(value.cur_buf_output_list) and len(value.cur_buf_output):
                ...
                gen_str += (
                    """#define %s_OUTPUT_IMPEDANCE\t\tCLK_BUF_OUTPUT_IMPEDANCE_%d\n"""
                    % (key, idx)
                )
        ...
        for key in sorted_key(list(ModuleObj.get_data(self).keys())):
            ...
            if len(value.cur_driving_control_list) and len(value.cur_driving_control):
                ...
                gen_str += (
                    """#define %s_CONTROLS_FOR_DESENSE\t\tCLK_BUF_CONTROLS_FOR_DESENSE_%d\n"""
                    % (key, idx)
                )
