...
from utility.util import LogLevel, compare, log
    ...
    def refGpio(self, eint_num, flag):
            ...
            if string.atoi(eint_num) == string.atoi(key):
                ...
                if flag:
                    for item in list(temp_map.keys()):
                        ...
                        if item_data.get_defMode() == string.atoi(
                            temp_map[item].split(":")[0]
                        ):
                            gpio_vec = []
        ...
        if gpio_num >= 0:
            ...
            if flag:
                ...
                if re.match(r"GPIO[\d]+", mode_name) or re.match(
                    r"EINT[\d]+", mode_name
                ):
                    return gpio_vec
    ...
    def fill_dtsiFile(self):
        ...
        for key in sorted_list:
            ...
            gen_str += """\tinterrupts = <%s %s %s %d>;\n""" % (
                ...
                temp,
                self.refGpio(key[4:], True)[0],
                self.refGpio_defMode(key[4:], True),
            )
