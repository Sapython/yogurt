...
from utility.util import LogLevel, compare, log, sorted_key
...
class PmicObj(ModuleObj):
    ...
    def fill_dtsiFile(self):
        ...
        for key in sorted_key(list(ModuleObj.get_data(self).keys())):
            ...
            gen_str += (
                """\tregulator-default-on = <%d>; /* 0:skip, 1: off, 2:on */\n"""
                % (value.get_defEnable())
            )
            gen_str += """\tstatus = \"okay\";\n"""
            gen_str += """};\n"""
        ...
        for key in sorted_key(list(ModuleObj.get_data(self).keys())):
            ...
            for varName in value.get_nameList():
                ...
                if varName.find("SUB_CAMERA") != -1:
                    gen_str += (
                        """\tvcam%s_main2-supply = <&mt_pmic_%s_ldo_reg>;\n"""
        ...
        for key in sorted_key(list(ModuleObj.get_data(self).keys())):
            ...
            for varName in value.get_nameList():
                ...
                if varName.find("MAIN_CAMERA_3") != -1:
                    gen_str += (
                        """\tvcam%s_main3-supply = <&mt_pmic_%s_ldo_reg>;\n"""
                        % (postFix.lower(), value.get_ldoName().lower())
                    )
